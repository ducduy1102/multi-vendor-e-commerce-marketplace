...
import { headers as getHedears } from "next/headers";

import { Category, Media, Tenant } from "@/payload-types";
import { baseProcedure, createTRPCRouter } from "@/trpc/init";
import { DEFAULT_LIMIT } from "@/constants";
import { sortValue } from "@/modules/products/search-params";
...
export const productsRouter = createTRPCRouter({
  getOne: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      const headers = await getHedears();
  ...
  getMany: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.tenantSlug) {
        ...
        // Make sure to not load products set to "isPrivate: true" (using reverse note_equals logic)
        // These products are exculusively private to the tenant store
